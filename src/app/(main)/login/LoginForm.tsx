...
function LoginButton() {
    ...
    return (
        <Button className="w-full h-12 rounded-2xl cursor-pointer font-bold text-xs uppercase tracking-widest bg-primary text-primary-foreground hover:opacity-90 transition-all duration-300" disabled={pending}>
...
export function LoginForm({ branding }: LoginFormProps) {
    ...
    const style = branding?.primaryColor
        ? { "--primary": hexToHsl(branding.primaryColor) } as React.CSSProperties
        : {};

    return (
        <div 
            style={style}
            className="flex h-screen w-full items-center justify-center bg-black relative overflow-hidden"
        >
            <div className="w-full max-w-sm relative z-10 px-4">
                <div className="mb-12 flex flex-col items-center justify-center">
                    ...
                    <h1 className="text-5xl font-black tracking-tighter text-white">
                        {branding?.name || "SOPP"}{!branding?.name && <span className="text-primary">.</span>}
                    </h1>
                    <div className="h-px w-12 bg-white/20 my-3" />
                    <p className="text-[8px] uppercase font-black tracking-[0.2em] text-white/40">
                        {branding?.name ? `Portal Exclusivo - ${branding.name}` : "Sistema Operacional de Performance Pessoal"}
                    </p>
                </div>

...
export async function fetchBibleText(reference: string, translation: string = DEFAULT_TRANSLATION): Promise<BibleResponse | null> {
  try {
    const encodedRef = encodeURIComponent(reference.replace(/ /g, "+"));

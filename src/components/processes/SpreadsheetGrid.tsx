...
export function SpreadsheetGrid({ list, people, projects, goals, currentUserId }: SpreadsheetGridProps) {
                            ...
                            <div className="flex-1 overflow-y-auto space-y-2.5 max-h-[600px] no-scrollbar">
                                {groupTasks.length === 0 ? (
                                    ...
                                    groupTasks.map((task: any) => {
                                        const rPerson = people.find(p => p.id === task.responsibleId);
                                                ...
                                                <div className="flex items-center justify-between gap-2 mb-2">
                                                    ...
                                                    {rPerson && (
                                                        <div className="flex items-center gap-1">
                                                            ...
                                                            <span className="text-[8px] uppercase font-black text-zinc-400 truncate max-w-[80px]">
                                                                {rPerson.name}
                                                            </span>
                                                        </div>
                                                    )}
                                ...
                                return (
                                    <div key={task.id} className="relative h-12 flex items-center">
                                        ...
                                        >
                                            ...
                                            {rPerson && (
                                                <span className="text-[7px] font-black bg-zinc-950/50 border border-white/5 rounded px-1 text-zinc-400 shrink-0 select-none uppercase">
                                                    {rPerson.name.split(" ")[0]}
                                                </span>
                                            )}
                                            ...
                                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover/bar:flex flex-col bg-zinc-950 border border-zinc-800 p-2.5 rounded-lg shadow-2xl z-50 min-w-[180px] pointer-events-none text-white">
                                                ...
                                                <div className="space-y-1">
                                                    ...
                                                    {rPerson && (
                                                        <div className="flex items-center justify-between text-[7px] uppercase font-bold text-zinc-550 pt-1 border-t border-zinc-900/50">
                                                            <span>Responsável:</span>
                                                            <span className="text-zinc-300">{rPerson.name}</span>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                ...
                                <table className="w-full border-collapse text-left text-xs min-w-[950px] overflow-hidden rounded-xl">
                                    <thead>
                                        <tr className="border-b border-zinc-800/80 bg-zinc-900/85 backdrop-blur-md text-zinc-300 font-black tracking-widest text-[9px] h-11 select-none uppercase">
                                            ...
                                            <th className="w-44 px-4 border-r border-zinc-800/50 bg-zinc-900/40 group/th cursor-pointer hover:bg-zinc-800/50 transition-colors select-none" onClick={() => handleSort("responsible")}>
                                                <div className="flex items-center">Responsável {renderSortIcon("responsible")}</div>
                                            </th>
                                    ...
                                    <tbody>
                                        ...
                                        {groupTasks.map((task: any) => {
                                            ...
                                            return (
                                                <tr key={task.id} className="border-b border-zinc-900/50 hover:bg-white/[0.01] group/row h-11 transition-all">
                                                    ...
                                                    {/* Assignee / Responsible selector */}
                                                    <td className="px-4 border-r border-zinc-900/40">
                                                        {isViewer ? (
                                                            <div className="w-full flex items-center justify-between px-2.5 py-1.5 text-left text-zinc-400">
                                                                <div className="flex items-center gap-2 truncate">
                                                                    <User className="w-3.5 h-3.5 text-zinc-700 shrink-0" />
                                                                    <span className="truncate max-w-[125px] font-bold text-xs">
                                                                        {task.responsible?.name || <span className="text-zinc-650 font-normal italic">Sem responsável</span>}
                                                                    </span>
                                                                </div>
                                                            </div>
                                                        ) : (
                                                            <DropdownMenu>
                                                                <DropdownMenuTrigger asChild>
                                                                    <button className="w-full flex items-center justify-between hover:bg-zinc-800/20 border border-transparent hover:border-zinc-800/40 rounded-lg px-2.5 py-1.5 transition-all focus:outline-none cursor-pointer text-left text-zinc-400 hover:text-zinc-200 group/cell">
                                                                        <div className="flex items-center gap-2 truncate">
                                                                            <User className="w-3.5 h-3.5 text-zinc-650 shrink-0 group-hover/cell:text-zinc-400" />
                                                                            <span className="truncate max-w-[125px] font-bold text-xs">
                                                                                {task.responsible?.name || <span className="text-zinc-655 font-normal italic group-hover/cell:text-zinc-500">Adicionar</span>}
                                                                            </span>
                                                                        </div>
                                                                        <ChevronDown className="w-3 h-3 text-zinc-650 shrink-0 ml-1 opacity-0 group-hover/cell:opacity-100 transition-opacity" />
                                                                    </button>
                                                                </DropdownMenuTrigger>
                                                                <DropdownMenuContent className="noir-glass border-zinc-800 max-h-60 overflow-y-auto w-56 p-1">
                                                                    <DropdownMenuItem onClick={() => handleUpdateTaskField(task.id, "responsibleId", null)} className="font-bold text-[10px] uppercase cursor-pointer rounded-md">
                                                                        Nenhum
                                                                    </DropdownMenuItem>
                                                                    {people.length === 0 ? (
                                                                        <div className="p-3 text-center">
                                                                            <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-wider mb-2">Nenhum colaborador cadastrado</p>
                                                                            <Button 
                                                                                size="sm" 
                                                                                className="bg-white text-black hover:bg-zinc-200 font-bold uppercase text-[9px] tracking-widest w-full py-1 h-7"
                                                                        ...
                                                                        people.map(p => (
                                                                            <DropdownMenuItem key={p.id} onClick={() => handleUpdateTaskField(task.id, "responsibleId", p.id)} className="font-bold text-[10px] uppercase cursor-pointer rounded-md">
                                                                                {p.name}
                                                                            </DropdownMenuItem>
                                                                        ))
                                                                    )}
                                        ...
                                        {isViewer ? (
                                            <tr className="h-10">
                                                <td colSpan={9 + customFields.length} className="px-4 py-2 text-center">
